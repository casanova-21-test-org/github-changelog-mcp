...
When creating GitHub issues for weekly reports, **always apply appropriate labels**:

#### **Required Labels Based on Report Content**
Apply labels based on the changelog categories present in the report:
- **`copilot`**: For Copilot changelog entries
- **`collaboration-tools`**: For Collaboration tools changelog entries
- **`actions`**: For Actions changelog entries
- **`application-security`**: For Application Security changelog entries
- **`platform-governance`**: For Platform governance changelog entries
- **`projects-issues`**: For Projects & Issues changelog entries
- **`supply-chain-security`**: For Supply chain security changelog entries
- **`ecosystem-accessibility`**: For Ecosystem & accessibility changelog entries
- **`community-engagement`**: For Community engagement changelog entries
- **`enterprise-management`**: For Enterprise management tools changelog entries
- **`account-management`**: For Account management changelog entries
- **`client-apps`**: For Client apps changelog entries

#### **Standard Report Labels**
- **`weekly-report`**: Always include for automated weekly reports
- **`changelog`**: Always include since these are changelog-based reports
- **`automation`**: Include for reports generated by GitHub Actions workflows

#### **Label Application Logic**
- Scan report content to identify relevant changelog categories
- Apply **all applicable** category labels (reports can contain multiple categories)
- Always include the standard report labels (`weekly-report`, `changelog`, `automation`)
- Example: A report with Copilot and Actions entries should have: `copilot`, `actions`, `weekly-report`, `changelog`, `automation`
- Example: A report with Security and Enterprise content should have: `application-security`, `enterprise-management`, `weekly-report`, `changelog`, `automation`
...
When instructing AI agents to create issues, specify labels based on content:
```
"Create a GitHub issue titled 'Weekly Copilot Report - 2024-10-10' with the formatted report as the body. Apply these labels: copilot, weekly-report, changelog, automation"
```

For mixed content reports:
```
"Create a GitHub issue with labels: copilot, actions, application-security, weekly-report, changelog, automation"
```

For enterprise-focused reports:
```
"Create a GitHub issue with labels: enterprise-management, account-management, platform-governance, weekly-report, changelog, automation"
```
...
9. **Missing Labels**: Always apply appropriate labels when creating GitHub issues (product-specific + standard report labels)
